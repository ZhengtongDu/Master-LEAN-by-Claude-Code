...
> **本章目标**：理解 LEAN 是什么，完成 LEAN 4 开发环境的搭建，并通过一个简单的 Toy Case 体验“定义”与“证明”的一体性。

## 1.1 什么是 LEAN？

LEAN（目前主流版本为 LEAN 4）不仅仅是一个**交互式定理证明器**（Interactive Theorem Prover），它同时也是一门功能强大的**函数式编程语言**。

对于具有高等数学背景和编程基础的你来说，可以这样理解 LEAN 的双重身份：

1.  **作为证明器**：它基于**依赖类型理论**（Dependent Type Theory）。在 LEAN 中，**命题（Proposition）是类型（Type），证明（Proof）是该类型的项（Term）**。这就是著名的“Curry-Howard 同构”。
2.  **作为编程语言**：它不仅能写数学证明，还能写高效的可执行代码。LEAN 4 自身的大部分组件（包括解析器、甚至部分编译器）都是用 LEAN 语言本身编写的，这意味着其元编程（Metaprogramming）能力极其强大。

### 为什么选择 LEAN 4？
* **数学库（Mathlib4）**：拥有庞大且活跃的数学形式化社区，涵盖了代数、拓扑、分析等广泛领域。
* **交互体验**：配合 VS Code 插件，提供实时的证明状态反馈（Infoview），让写证明像写代码一样有即时的 Debug 体验。
* **可扩展性**：你可以编写自己的 Tactic（策略）来自动化繁琐的证明步骤。

---

## 1.2 环境搭建：工欲善其事

LEAN 4 的标准开发环境组合是：**VS Code + Elan (版本管理器) + Lake (构建工具)**。
...
2.  安装 `git`。
3.  在 PowerShell 中运行官方安装脚本（这将安装 Elan）：
    ```powershell
    curl [https://raw.githubusercontent.com/leanprover/elan/master/elan-init.ps1](https://raw.githubusercontent.com/leanprover/elan/master/elan-init.ps1) -o elan-init.ps1
    Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser
    .\elan-init.ps1
    ```
4.  打开 VS Code，安装插件 **"lean4"**。
...
### 1.2.2 创建你的第一个项目
LEAN 4 使用 `lake`（Lean Make）作为包管理器和构建工具。
打开终端，执行以下命令创建一个新项目：

```bash
# 创建一个名为 hello_lean 的项目
lake new hello_lean
...
- `lakefile.lean`: 项目配置文件（依赖管理）。
- `HelloLean.lean` (或 `Main.lean`): 源代码文件。
💡 Usage Tip: 打开 .lean 文件后，VS Code 右侧会自动弹出 Lean Infoview 面板。如果未弹出，请点击右上角的 "∀" 图标。Infoview 是你与 LEAN 交流的窗口，务必时刻关注它显示的 Goal（当前证明目标）和 Error（错误信息）。
---

## 1.3 LEAN 初体验：Toy Case
让我们在 VS Code 中创建一个新文件 `Chapter01_Basic.lean`，动手写下第一行 LEAN 代码。
...
#eval "Hello, " ++ "World!" -- Infoview 输出: "Hello, World!"

-- 定义一个简单的函数
def f (x : Nat) := x + 3

#eval f 5         -- Infoview 输出: 8

```

### 1.3.2 第一个定理证明
现在，我们来证明一个非常简单的数学命题：$1 + 1 = 2$。
在 LEAN 中，我们使用 `example` 关键字来声明一个匿名的定理，或使用 `theorem` 关键字声明一个具名定理。
...
### 1.3.3 命题即类型（Propositions as Types）
...
### 1.3.4 函数与蕴含
在逻辑中，$P \implies Q$（如果 P 成立，则 Q 成立）。
在类型论中，这对应于一个**函数**：输入一个 P 的证明，输出一个 Q 的证明。
让我们看一个稍微复杂一点的 Toy Case：

```lean
variable (P Q : Prop)

-- 假设我们有一个证明 hp : P
-- 假设我们有一个推导规则 himp : P → Q
-- 我们要证明 Q 成立
example (hp : P) (himp : P → Q) : Q := by
  -- 我们有一个函数 himp，它能把 P 变成 Q
  -- 我们有一个参数 hp，它是 P
  -- 所以 himp hp 就是 Q
  exact himp hp

```
💡 Usage Tip: 在 by ... 代码块中输入代码时，你可以尝试把光标停在不同行。Infoview 会显示由于执行了当前行指令后系统的状态。学会通过移动光标来“调试”证明逻辑，是精通 LEAN 的第一步。
## 1.4 本章小结

1. **安装**：通过 `elan` 和 VS Code 插件搭建环境。
2. **Infoview**：这是你的仪表盘，写代码时视线要在这和代码编辑器之间来回切换。
3. **核心直觉**：
  - `example : A := t` 意味着 `t` 是命题 `A` 的证明。
  - 证明过程本质上是构造一个特定类型的项（Term）。
  - `rfl` 处理的是定义上的相等（Definitional Equality）。
在下一章中，我们将深入探讨 LEAN 的基础语法与逻辑连接词，开始构建真正的数学证明。
...
这是为您大幅扩写后的 `chapter01.md`。
针对您（计算机专业博士生）的背景，我在扩写中增加了以下深度内容：

1. **底层原理**：更深入地解释了 Curry-Howard 同构和依赖类型理论（Dependent Type Theory）。
2. **双模式对比**：明确对比了“项模式”（Term Mode）和“策略模式”（Tactic Mode），这是理解 LEAN 的关键。
3. **编程与证明的统一**：增加了作为编程语言的 Toy Case，展示 LEAN 如何像 Haskell/OCaml 一样工作。
4. **实战细节**：补充了项目结构解析和常见排错技巧。
...
* **元编程（Metaprogramming）**：LEAN 4 是“自举”的（Self-hosted）。你可以用 LEAN 语言本身去写 LEAN 的扩展、Tactics（策略）甚至 DSL。这对于计算机专业的学生来说，意味着极高的可玩性和工具定制能力。
...
⚠️ 关键提示： 打开 .lean 文件后，请紧盯 VS Code 右侧。你应该会看到 Lean Infoview 面板自动弹出。如果没有，点击右上角的 "∀" 图标手动打开。 Infoview 是你的“示波器”，它实时显示当前的证明状态（Goal）和错误信息。
---

## 1.4 Toy Case 1：LEAN 作为编程语言
在开始证明之前，先感受一下 LEAN 作为一门强类型函数式语言的特性。这有助于你理解“项”（Term）的构造。
在 `HelloProof.lean` 中输入以下代码：
...
```
**重要区别**：
LEAN 具有 **Proof Irrelevance（证明无关性）**。

- 对于数据（`Type`），我们关心它是哪个值（`5` 和 `6` 是不同的）。
- 对于命题（`Prop`），我们只关心它**是否有**证明。如果有两个不同的证明 `h1 : P` 和 `h2 : P`，在逻辑上我们将它们视为等价的。这一特性对于自动化证明至关重要。
...
下一章，我们将深入探讨命题逻辑中的 `And`（与）、`Or`（或）与 `Not`（非），并学习如何在 LEAN 中处理它们。
